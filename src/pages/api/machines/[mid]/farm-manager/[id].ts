...
import { assignFarmManagerMachine } from '@/services/machine/query/assign-machine';
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'PUT') {
      // PUT /api/machines/${mid}/farm-manager/${id}
...
async function assignMachine(mid: string, id: string) {
  try {
    const machine = await assignFarmManagerMachine(mid, id);
    return machine;
  } catch (error) {
    throw error;
  }
